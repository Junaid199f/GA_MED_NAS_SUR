...
OPS = {
    ...
    'inv_res_3x3': lambda C, stride, affine: InvertedResidual(C, C, 3, stride, expand_ratio=1, affine=affine,
                                                              shuffle=False),
    'inv_res_5x5': lambda C, stride, affine: InvertedResidual(C, C, 5, stride, expand_ratio=1, affine=affine,
                                                              shuffle=False),

    'inv_res_3x3_sh': lambda C, stride, affine: InvertedResidual(C, C, 3, stride, expand_ratio=1, affine=affine,
                                                                 shuffle=True),
    'inv_res_5x5_sh': lambda C, stride, affine: InvertedResidual(C, C, 5, stride, expand_ratio=1, affine=affine,
                                                                 shuffle=True),

    # 'std_gn_3x3' : lambda C, stride, affine: ReLUStdConvGN(C, C, 3, stride, 1, affine=affine),
    'std_gn_3x3': lambda C, stride, affine: ActConvBN(C, C, 3, stride, affine=affine, preact=True, conv_layer=StdConv2d,
                                                      norm_layer=nn.GroupNorm, act_layer=nn.ReLU),

    # 'std_gn_5x5' : lambda C, stride, affine: ReLUStdConvGN(C, C, 5, stride, 2, affine=affine),
    'std_gn_5x5': lambda C, stride, affine: ActConvBN(C, C, 5, stride, affine=affine, preact=True, conv_layer=StdConv2d,
                                                      norm_layer=nn.GroupNorm, act_layer=nn.ReLU),

    # 'std_gn_7x7' : lambda C, stride, affine: ReLUStdConvGN(C, C, 7, stride, 3, affine=affine),
    'std_gn_7x7': lambda C, stride, affine: ActConvBN(C, C, 7, stride, affine=affine, preact=True, conv_layer=StdConv2d,
                                                      norm_layer=nn.GroupNorm, act_layer=nn.ReLU),
    ...
    'octave_conv_3x3': lambda C, stride, affine: ReLU_OctaveConv_BN(C, C, kernel_size=3, stride=stride, padding=1,
                                                                    affine=affine, alpha_in=0, alpha_out=0),
    'octave_conv_5x5': lambda C, stride, affine: ReLU_OctaveConv_BN(C, C, kernel_size=5, stride=stride, padding=2,
                                                                    affine=affine, alpha_in=0, alpha_out=0),
...
class ActConvBN(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, dilation=1, groups=1, affine=True, preact=True,
                 conv_layer=nn.Conv2d, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d):
        ...
        self.conv = conv_layer(C_in, C_out, kernel_size, stride=stride, padding=padding, dilation=dilation,
...
class InvertedResidual(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, expand_ratio, affine=True, shuffle=False, preact=True,
                 act_layer=nn.ReLU):
        ...
        self.op = nn.Sequential(
            # pw
            ActConvBN(C_in, C_in * self.expand_ratio, 1, 1, affine=affine, preact=preact, conv_layer=nn.Conv2d,
                      norm_layer=nn.BatchNorm2d, act_layer=act_layer),
            # dw
            ActConvBN(C_in * self.expand_ratio, C_in * self.expand_ratio, kernel_size, stride,
                      groups=C_in * self.expand_ratio, affine=affine, preact=preact, conv_layer=nn.Conv2d,
                      norm_layer=nn.BatchNorm2d, act_layer=act_layer),
            # pw-linear
            ActConvBN(C_in * self.expand_ratio, C_out, 1, 1, affine=True, preact=preact, conv_layer=nn.Conv2d,
...
class MBConv(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride, padding, t=3, affine=True, use_se=False, preact=True,
                 act_layer=nn.ReLU):
        super(MBConv, self).__init__()
        self.t = t
        if self.t > 1:
            self._expand_conv = ActConvBN(C_in, C_in * self.t, 1, 1, groups=1, affine=affine, preact=preact,
                                          conv_layer=nn.Conv2d, norm_layer=nn.BatchNorm2d, act_layer=act_layer)

            self._depthwise_conv = ActConvBN(C_in * self.t, C_in * self.t, kernel_size, stride, groups=C_in * self.t,
                                             affine=affine, preact=preact, conv_layer=nn.Conv2d,
                                             norm_layer=nn.BatchNorm2d, act_layer=act_layer)

            self._project_conv = ActConvBN(C_in * self.t, C_out, 1, 1, groups=1, affine=affine, preact=preact,
                                           conv_layer=nn.Conv2d, norm_layer=nn.BatchNorm2d, act_layer=None)
        else:
            self._expand_conv = None

            self._depthwise_conv = ActConvBN(C_in, C_in, kernel_size, stride, groups=C_in, affine=affine, preact=preact,
                                             conv_layer=nn.Conv2d, norm_layer=nn.BatchNorm2d, act_layer=act_layer)

            self._project_conv = ActConvBN(C_in, C_out, 1, 1, groups=1, affine=affine, preact=preact,
                                           conv_layer=nn.Conv2d, norm_layer=nn.BatchNorm2d, act_layer=None)
