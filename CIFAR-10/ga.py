...
class GA(Optimizer):
  def __init__(self,population_size,number_of_generations,crossover_prob,mutation_prob,blocks_size,num_classes,in_channels,epochs,batch_size,layers,n_channels,dropout_rate,retrain,resume_train,cutout,multigpu_num,grad_clip,type_crossover):
    super().__init__(population_size,number_of_generations,crossover_prob,mutation_prob,blocks_size,num_classes,in_channels,epochs,batch_size,layers,n_channels,dropout_rate,retrain,resume_train,cutout,multigpu_num,grad_clip,type_crossover)
  ...
  def evolve(self):
    ...
    for i in range(gen,self.number_of_generations-1,1):
      ...
      #Step 8: Get fitness values of two offsprings
        ...
        logging.info("Time taken by surrogate to predict the fitness of the model %s", time.process_time() - start)
  ...
  def evaluate_single_model(self,indv):
    ...
    loss = self.evaluator.train_final(network, 2500, hash_indv,self.grad_clip)
    print("loss",loss)
